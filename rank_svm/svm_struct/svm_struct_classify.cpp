...
int svm_struct_classify_main (int argc, char* argv[], SAMPLE &testsample, bool read_data)
{
  ...
  if(read_data)
	  ...
	  partial_read_struct_examples(testsample, &sparm);
  ...
  if((!no_accuracy) && (struct_verbosity>=1)) {
    printf("Average lmodeloss on test set: %.4f\n",(float)avgloss);
