...
void   read_input_parameters_learn(int, char **, char *, char *,long *, long *,
			     STRUCT_LEARN_PARM *, LEARN_PARM *, KERNEL_PARM *,
			     int *);
void   wait_any_key();
void   print_help_learn();

int  svm_struct_main (int argc, char* argv[], SAMPLE &sample, bool read_data)
{  
  ...
  read_input_parameters_learn(argc,argv,trainfile,modelfile,&verbosity,
			&struct_verbosity,&struct_parm,&learn_parm,
			&kernel_parm,&alg_type);

   if(struct_verbosity>=1) {//struct_verbosity->read
...
void read_input_parameters_learn(int argc,char *argv[],char *trainfile,
			   char *modelfile,
			   ...
			   int *alg_type)
{
  ...
  if(learn_parm->rho<0) {
    ...
    printf("be greater than zero (typicallyalg_type 1.0 or 2.0, see T. Joachims, Estimating the\n");
...
void svm_model_initialization(SAMPLE *sample, STRUCTMODEL *sm){
  ...
  sm->svm_model->supvec = (DOC **)my_malloc(sizeof(DOC *)*(totdoc+2));
  sm->svm_model->alpha = (double *)my_malloc(sizeof(double)*(totdoc+2));
  sm->svm_model->index = (long *)my_malloc(sizeof(long)*(totdoc+2));

  sm->svm_model->at_upper_bound=0;
  sm->svm_model->b=0;	       
  sm->svm_model->supvec[0]=0;  /* element 0 reserved and empty for now */
  sm->svm_model->alpha[0]=0;
  sm->svm_model->lin_weights=NULL;
  sm->svm_model->totwords=sm->sizePsi+1;
  sm->svm_model->totdoc=totdoc;
  sm->svm_model->kernel_parm=kernel_parm;
  sm->svm_model->sv_num=1;
  sm->svm_model->loo_error=-1;
  sm->svm_model->loo_recall=-1;
  sm->svm_model->loo_precision=-1;
  sm->svm_model->xa_error=-1;
  sm->svm_model->xa_recall=-1;
  sm->svm_model->xa_precision=-1;
